...
def load_data(cfg):
    ...
    if cfg.DATASETS.DATASET_MODE == 'cityscapes':
        ...
        all_files = _list_image_files_recursively(
            os.path.join(cfg.DATASETS.DATADIR, 'images', 'train' if cfg.TRAIN.IS_TRAIN else 'val'))
        classes = _list_image_files_recursively(
            os.path.join(cfg.DATASETS.DATADIR, 'annotations', 'train' if cfg.TRAIN.IS_TRAIN else 'val'))
        instances = None
